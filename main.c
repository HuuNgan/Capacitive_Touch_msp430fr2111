...
//#define NUMBER_COUNT_CAPTURES        5
#define NUMBER_OF_MODES              7
#define TOUCH_DETECTED_VALUE         105
//#define DEACTIVE_VALUE               101
//long int timer_count;
long int IC_val1, IC_val2;
int mode = 0, periode;
//volatile unsigned int Periode_array[NUMBER_COUNT_CAPTURES];
//unsigned int Periode_pointer = 0;
...
int main(void)
{
	...
	configClock();
//  cap_touch_init();
	CAPTIOCTL |= (CAPTIOEN | CAPTIOPOSEL_2 | CAPTIOPISEL_1);        // enable cap touch at P2.1
...
//    P1DIR |= BIT0;
    ...
    P2IE  |= CapT1 + CapT2;             // interrupt enabled
    P2DIR &= ~(CapT1);          // set CapT1 and CapT2 as input
